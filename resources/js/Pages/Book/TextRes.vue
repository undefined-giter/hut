<template>
  <div class="mb-6">
    <p id="p1" class="mb-6 text-lg text-justify p1-left p1-active">&emsp;&emsp;Bienvenue dans notre cabane de luxe située au cœur de la nature. Offrez-vous un séjour inoubliable dans une cabane équipée d'un <span class="highlight">jacuzzi privatif</span>, parfaite pour un moment de détente loin du stress quotidien. Profitez d'une <span class="highlight">vue à couper le souffle</span> tout en étant entouré par la beauté de la nature. Notre cabane est idéale pour une <span class="highlight">escapade romantique</span>, un <span class="highlight">week-end détente</span> ou encore une petite pause bien méritée.</p>

    <p id="p2" class="text-lg text-justify p2-top p2-active">&emsp;&emsp;Vous avez le choix entre une location pour une nuit ou plusieurs nuits pour un séjour prolongé. Nos tarifs s'adaptent en fonction de la durée de votre séjour : <span class="highlight">{{ PRICE_PER_NIGHT }}€/nuit</span> pour une nuit et <span class="highlight">{{ PRICE_PER_NIGHT_FOR_2_AND_MORE_NIGHTS }}€/nuit</span> pour les séjours de deux nuits ou plus.<span v-show="PERCENT_REDUCED_WEEK !== 0"> De plus, il y a une <span class="highlight">{{ PERCENT_REDUCED_WEEK > 0 ? 'réduction de ' + PERCENT_REDUCED_WEEK : 'augmentation de +' + String(PERCENT_REDUCED_WEEK).slice(1) }}% pour les nuits de semaine</span>, de lundi 14h à vendredi 12h.</span> Le parking est inclus pour toute réservation, ainsi que les taxes d'habitation et de séjour et équipements de sécurité.</p>
  </div>
